...
class Hazid(Base):  # type: ignore
    __tablename__ = "hazid_monitoring"
    ...
    guideword: SQLAlchemyMapped[str | None] = sqlalchemy_mapped_column(sqlalchemy.String(length=255), nullable=True)
    hazard: SQLAlchemyMapped[str | None] = sqlalchemy_mapped_column(sqlalchemy.String(length=None), nullable=True)
    ...
    field_location: SQLAlchemyMapped[typing.Optional["FieldLocation"]] = relationship(
        "FieldLocation", back_populates="hazid_monitorings", lazy="selectin"
    )
