...
class AccountCRUDRepository(BaseCRUDRepository):
    async def create_account(self, account_create: AccountInCreate) -> Account:
        new_account = Account(name=account_create.name, email=account_create.email, is_logged_in=True)
    ...
    async def read_account_by_id(self, id: str) -> Account:
    ...
    async def read_account_by_name(self, name: str) -> Account:
        stmt = sqlalchemy.select(Account).where(Account.name == name)
        query = await self.async_session.execute(statement=stmt)
        db_account = query.scalar()

        if not db_account:
            raise EntityDoesNotExist("Account with name `{name}` does not exist!")
    ...
    async def update_account_by_id(self, id: str, account_update: AccountInUpdate) -> Account:
        ...
        if new_account_data["name"]:
            update_stmt = update_stmt.values(name=new_account_data["name"])
    ...
    async def delete_account_by_id(self, id: str) -> str:
        select_stmt = sqlalchemy.select(Account).where(Account.id == id)
        query = await self.async_session.execute(statement=select_stmt)
    ...
    async def is_name_taken(self, name: str) -> bool:
        ...
        if db_name:
            raise EntityAlreadyExists(f"The name `{name}` is already registered!")
