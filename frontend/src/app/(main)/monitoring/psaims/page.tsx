...
const PSAIMS_TYPE_OPTIONS = [
	{ value: "zona_indicator", label: "Zona Indicator" },
	{ value: "zona_pse_list", label: "Zona PSE List" },
];

const EMPTY_DATA: any[] = [];

export default function PsaimsMonitoringPage() {
	const router = useRouter();
	const [activeType, setActiveType] = useState<"zona_indicator" | "zona_pse_list">("zona_indicator");
	const [selectedRow, setSelectedRow] = useState<any | null>(null);

	// Zona filter
	const [tempZona, setTempZona] = useState<string>("");
	const [tempYear, setTempYear] = useState(YEAR_OPTIONS[0]);
	const [tempMonth, setTempMonth] = useState(MONTH_OPTIONS[0]);

	const [queryZona, setQueryZona] = useState<string>("");
	...
	const { data = EMPTY_DATA, isLoading } = useMonitoringData(activeType, {
		...
		month: isZonaIndicator ? null : queryMonth.value,
		zona: queryZona || null,
	});

	const handleApply = () => {
		setQueryZona(tempZona);
			...
			return (
				<button
					...
					className={`w-9 h-9 flex items-center justify-center rounded-lg border text-sm font-semibold transition-all ${
						isActive ? "border-[#008A45] text-[#008A45] bg-[#008A45]/5 font-bold" : "border-gray-200 text-gray-600 hover:bg-gray-50 bg-white"
					}`}
				>
					{p + 1}
				</button>
	...
	return (
		<div className="flex flex-col gap-5 p-6 w-full min-w-0">
			...
			<div className="flex items-start justify-between">
				...
				<Button
					variant="outline"
					onClick={() => router.push(`/monitoring/${activeType}/history`)}
			...
			<div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
				...
				<div className="flex flex-wrap items-end gap-4">
					...
					{/* Month — only for Zona PSE List */}
					{!isZonaIndicator && (
						<div className="flex-1 min-w-[150px]">
							<label className="block text-xs font-semibold text-gray-500 mb-1.5 ml-0.5">Bulan</label>
							<Select
							...
							{isLoading ? (
								Array.from({ length: 5 }).map((_, idx) => (
									<tr key={idx} className="animate-pulse">
										{columns.map((_, cidx) => (
											<td key={cidx} className="px-4 py-4"><div className="h-4 bg-gray-100 rounded w-full" /></td>
										))}
									</tr>
								))
							) : data.length === 0 ? (
			...
			<DataGatheringDetailModal
				docType={activeType === "zona_indicator" ? "ZONA_INDICATOR" : "ZONA_PSE_LIST"}
